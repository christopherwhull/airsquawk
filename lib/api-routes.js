            ...
            (async () => {
                try {
                    ...
                    const hourlyFiles = await listS3Files(s3, writeBucket, 'data/hourly/positions_');
                    let minuteCount = 0, hourlyCount = 0;
                    for (const file of minuteFiles) {
                        try {
                            const data = await downloadAndParseS3File(s3, writeBucket, file.Key);
        ...
        (async () => {
            try {
                const readFiles = await listS3Files(s3, readBucket, 'data/piaware_aircraft_log');
                const writeFiles = await listS3Files(s3, writeBucket, 'data/piaware_aircraft_log');
                let readCount = 0, writeCount = 0;
                for (const file of readFiles) {
                    try {
                        const data = await downloadAndParseS3File(s3, readBucket, file.Key);
    ...
    (async () => {
        try {
            logger.info('[restart] Restart request arrived');
            const readFiles = await listS3Files(s3, readBucket, 'data/piaware_aircraft_log');
            const writeFiles = await listS3Files(s3, writeBucket, 'data/piaware_aircraft_log');
...
async function loadAllHeatmapPositions(s3, readBucket) {
    ...
    // Get all files
    const files = await listS3Files(s3, readBucket, 'data/piaware_aircraft_log');
    console.log(`Found ${files.length} S3 files to load`);
...
function setupApiRoutes(app, s3, readBucket, writeBucket, getInMemoryState, cache = {}, positionCache = null) {
    ...
    app.get('/api/positions', async (req, res) => {
        try {
            const hours = parseInt(req.query.hours || '24', 10);
            const positions = positionCache?.getPositionsByTimeWindow(hours) || [];

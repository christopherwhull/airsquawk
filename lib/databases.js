...
async function getAirlineDatabase(s3, bucketName) {
    ...
    // If no S3 client or bucket configured, skip S3 fetch and use local fallback directly
    if (!s3 || !bucketName) {
        logger.info('S3 not configured; skipping S3 fetch for airline database, using local file if available');
        try {
    ...
    try {
        ...
        // Fallback to local file (if S3 fetch failed despite having S3 configured)
        try {
            const localPath = path.join(__dirname, '..', 'airline_database.json');
            if (fs.existsSync(localPath)) {
                logger.info('Loading airline database from local file...');
...
async function getAircraftTypesDatabase(s3, bucketName) {
    ...
    // If no S3 client or bucket configured, skip S3 fetch and use local fallback directly
    if (!s3 || !bucketName) {
        logger.info('S3 not configured; skipping S3 fetch for aircraft types database, using local file if available');
        try {
            const localPath = path.join(__dirname, '..', 'aircraft_types.json');
            if (fs.existsSync(localPath)) {
                logger.info('Loading aircraft types database from local file...');
                const fileContent = fs.readFileSync(localPath, 'utf-8');
                aircraftTypesDatabase = JSON.parse(fileContent);
                lastFetchTime = now;
                logger.info(`Successfully loaded aircraft types database from local file. Found ${Object.keys(aircraftTypesDatabase.types || {}).length} types.`);
                return aircraftTypesDatabase;
            }
        } catch (localErr) {
            logger.error('Failed to load from local file:', localErr.message);

        ...
        // Official FAA IFR Low Chart from ArcGIS
        const faaIfrLow = L.tileLayer('http://localhost:3003/tile/IFR_AreaLow/{z}/{y}/{x}', {
            attribution: '© FAA',
            maxZoom: 12,
            minZoom: 7,
            zIndex: 200,
        ...
        const faaVfrSectional = L.tileLayer('http://localhost:3003/tile/VFR_Sectional/{z}/{y}/{x}', {
            attribution: '© FAA',
            maxZoom: 12,
            minZoom: 7,
            zIndex: 200,
        ...
        const faaVfrTerminal = L.tileLayer('http://localhost:3003/tile/VFR_Terminal/{z}/{y}/{x}', {
            attribution: '© FAA',
            maxZoom: 12,
            minZoom: 7,
            zIndex: 200,
        ...
        const overlayLayers = {
            "FAA IFR Low": faaIfrLow,
            "FAA IFR High": faaIfrHigh,
            "FAA VFR Sectional": faaVfrSectional,
            "FAA VFR Terminal Area": faaVfrTerminal
        };
        ...
        function createAircraftIcon(color = '#ff3300', size = 20, rotation = 0) {
            const svg = `
                <svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24">
                    <path d="M21 11.5c0-.28-.22-.5-.5-.5h-6.79L12 4.21 10.29 5.92 11.5 7.12 8.75 9.88 7.34 9.5c-.14-.03-.28-.02-.41.04L4 10.5c-.44.14-.75.55-.75 1v1c0 .45.31.86.75 1l2.93.84c.13.04.27.06.41.04l1.41-.38L11.5 16.9 10.29 18.1 12 19.79l1.71-1.71H21.5c.28 0 .5-.22.5-.5v-5.08z" fill="${color}" />
                </svg>`;
            const html = `<div style="transform: rotate(${rotation}deg); width: ${size}px; height:${size}px; display:flex; align-items:center; justify-content:center">${svg}</div>`;
            return L.divIcon({ html, className: 'aircraft-icon', iconSize: [size, size], iconAnchor: [size/2, size/2] });
        }

        // Compute icon rotation so the nose points 180 degrees from track
        ...
        function createAircraftLogoIcon(info = {}, rotation = 0, size = 28, verticalRate = 0) {
            ...
            let src = null;
            try {
                if (info.manufacturerLogo && typeof info.manufacturerLogo === 'string') src = info.manufacturerLogo;
                else if (info.manufacturer && typeof info.manufacturer === 'string') src = `/api/v2logos/${encodeURIComponent(info.manufacturer)}`;
        ...
        style.innerHTML = `.aircraft-icon svg { display:block; } .aircraft-icon { pointer-events: auto; } .aircraft-logo-icon img { display:block; } .aircraft-logo-icon { pointer-events:auto; }`;
        ...
        async function fetchLivePositions() {
            try {
                ...
                positions.forEach(p => {
                    ...
                    const icon = createAircraftIcon('#ff3300', 20, rot || 0);
                    ...
                    // Click to persist (if checkbox enabled) — toggle persisted track for this hex
                        ...
                        try {
                            ...
                            try {
                                ...
                                if (points.length > 0) {
                                    ...
                                    // color persist polyline by climb/descent if altitude available
                                    let color = '#ffaa00';
                                    const firstAlt = points[0] && typeof points[0].alt === 'number' ? points[0].alt : null;
                                    const lastAlt = points[points.length - 1] && typeof points[points.length - 1].alt === 'number' ? points[points.length - 1].alt : null;
                                    if (firstAlt !== null && lastAlt !== null) {

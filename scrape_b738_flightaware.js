...
function scrapeFlightAwareB738() {
    const baseUrl = 'https://www.flightaware.com/live/aircrafttype/B738';
    ...
    console.log('Starting to scrape B738 aircraft from FlightAware...');

    function processPage() {
        if (!hasMoreData || totalPages >= 200) {
            const registrationArray = Array.from(registrations).sort();
            console.log(`\nTotal unique B738 registrations found: ${registrationArray.length}`);
            ...
            fs.writeFileSync('b738_flightaware_registrations.json', JSON.stringify(output, null, 2));
            console.log('Saved registrations to b738_flightaware_registrations.json');
            return;
        }
